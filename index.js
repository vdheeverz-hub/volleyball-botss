...
async function handleCommand(interaction) {
  ...
  if (commandName === 'queue') {
    ...
    // Avoid immediately rematching the same two teams that just played each other.
    ...
    let idxA = Math.floor(Math.random() * data.queues[mode].length);
    let teamAKey = data.queues[mode][idxA];
    let idxB, teamBKey, attempts = 0;
    do {
      idxB = Math.floor(Math.random() * data.queues[mode].length);
      teamBKey = data.queues[mode][idxB];
      attempts++;
    } while (
      (idxB === idxA || recentPairs.includes([teamAKey, teamBKey].sort().join('|'))) &&
      attempts < 10
    );
    data.queues[mode].splice(Math.max(idxA, idxB), 1);
    data.queues[mode].splice(Math.min(idxA, idxB), 1);
    saveData(data);
    ...
    data.queues[mode].push(key);
    saveData(data);
    await interaction.reply(`Added team ${players.map(id => `<@${id}>`).join(' & ')} to the ${mode} queue.`);
  }
    ...
    for (const key in data.teams) {
      ...
      for (const playerId of t.players) {
        if (!highestByPlayer[playerId] || t.rating > highestByPlayer[playerId]) {
          highestByPlayer[playerId] = t.rating;
        }
      }
    }
    const entries = Object.entries(highestByPlayer).sort((a, b) => b[1] - a[1]).slice(0, 15);
    if (entries.length === 0) {
      await interaction.reply('No matches recorded yet.');
      return;
    }
    const lines = entries.map(([id, rating], i) => `**${i + 1}.** <@${id}> — ${rating} (${roleName(getRankForRating(rating))})`);
    const embed = new EmbedBuilder().setTitle('Top players (by highest rating)').setDescription(lines.join('\n')).setColor(0xff5c39);
    await interaction.reply({ embeds: [embed] });
  }
    ...
    saveData(data);
    await interaction.reply(`Match ${matchId} cancelled. No ratings were changed.`);
    const channel = await interaction.guild.channels.fetch(match.channelId).catch(() => null);
    if (channel) await channel.send('This match has been cancelled by an admin. Run `/closechannel` to remove this channel.').catch(() => {});
  }
    ...
    saveData(data);
    await interaction.reply('Season reset. All ratings are back to 1000, win/loss counts cleared. Match history is preserved.');
  }
    ...
    const mode = interaction.options.getString('mode');
    const keys = data.queues[mode];
    if (!keys || keys.length === 0) {
      await interaction.reply(`No teams currently waiting in the ${mode} queue.`);
      return;
    }
    const lines = keys.map((key, i) => `**${i + 1}.** ${teamLabel(data.teams[key])} (${data.teams[key].rating})`);
    const embed = new EmbedBuilder()
      .setTitle(`${mode} queue — ${keys.length} team(s) waiting`)
      .setDescription(lines.join('\n'))
      .setColor(0x378ADD);
    await interaction.reply({ embeds: [embed] });
  }
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        { name: 'Matches (Admin)', value: '`/startqueue` `/resolve` `/kickqueue` `/editteam` `/forcequeue` `/cancelmatch` `/seasonreset`' },
        { name: 'Matches (Everyone)', value: '`/closechannel`' },
        { name: 'Stats', value: '`/leaderboard` `/stats` `/profile` `/matchhistory` `/topplayers`' },
        { name: 'Moderation (Admin)', value: '`/clear` `/mute` `/unmute` `/kick` `/ban` `/unban` `/warn` `/warnings` `/lock` `/unlock` `/setmodlog`' }
      );
    await interaction.reply({ embeds: [embed] });
  }
